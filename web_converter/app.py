...
def detect_input_format(text):
    ...
    if re.match(r'^\s*<(!DOCTYPE|html|head|body|div|table|p|ul|ol|li|span|h[1-6])', text_stripped, re.IGNORECASE):
        return 'html'
    return 'markdown'
...
def convert():
    ...
    try:
        ...
        if output_fmt in ('docx', 'pdf'):
            ...
            tmp_output = tempfile.NamedTemporaryFile(suffix=f'.{output_fmt}', delete=False)
            tmp_output.close()
            subprocess.run([PANDOC_PATH, tmp_input, '-f', from_fmt, '-t', output_fmt, '-o', tmp_output.name], check=True)
            return send_file(tmp_output.name, as_attachment=True, download_name=f'converted.{output_fmt}')
